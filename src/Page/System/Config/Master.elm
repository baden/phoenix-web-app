    ...
    , div [ class "config-select" ]
        [ it M_TRACK_MIN
            [ div [ class "config-select-content" ]
                ...
                , span [ class "item-text" ] [ text <| t "config.Максимальное время работы в режиме Поиск - ", text <| t "config.7 суток", text "." ]
                , span [ class "item-text" ] [ text <| t "config.Рекомендуется для охраны автомобиля на длительной стоянке." ]
                ]
            ]
        , it M_TRACK_MID
            [ div [ class "config-select-content" ]
                [ span [ class "item-title" ] [ text <| t "config.Оптимально" ]
                , span [ class "item-text" ] [ text <| t "config.Максимальное время работы в режиме Поиск - ", text <| t "config.24 часа", text "." ]
                , span [ class "item-text" ] [ text <| t "config.Рекомендуется для наиболее вероятного обнаружения угнанного автомобиля." ]
                ]
            ]
        , it M_TRACK_MAX
            [ div [ class "config-select-content" ]
                [ span [ class "item-title" ] [ text <| t "config.Минимально" ]
                , span [ class "item-text" ] [ text <| t "config.Максимальное время работы в режиме Поиск - ", text <| t "config.6 часов", text "." ]
                , span [ class "item-text" ] [ text <| t "config.Рекомендуется для максимальной экономии энергии батареи питания." ]
                ]
            ]
        ]
