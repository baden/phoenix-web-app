...
warnMgs =
    [ row
        [ p [] [ UI.text "Предупреждение!" ]
        , p [] [ UI.text " Необдуманное изменение настроек может привести к полной неработоспособности Феникса. Изменяйте параметры только если полностью уверены в своих действиях." ]
        ]
    ]
...
errorMsg =
    [ row [ UI.text "Ошибка загрузки или данные от Феникса еще не получены." ] ]
